...
'''
@file run_pixel_images.py
@author Scott L. Williams, in collaboration with Elisa Piispa, Israel Pineda, María Solis-Aulestia, Cristina Yánez
@package ETo
@brief run a series of pixel image plots.
@LICENSE
# 
#  run_pixel_images.py Copyright (C) 2020-2026 Scott L. Williams.
# 
...
# generate pixel images from sample points already created by sample_pix

run_pixel_image_copyright = 'run_pixel_images.py Copyright (c) 2020-2026 Scott L. Williams, released under GNU GPL V3.0'
...
outdir = '../../data/graphics/2D/2022/plots/images/' + prefix[:-1] + \
    '/' + sday + '_' + eday + '/'

#thead = '"Constructed Binary Image\nShowing Daily Weather Class"'
thead = '"Column Smoothed Image\n(Hamming 9 Pixels)"'
#thead = '"Row Smoothed Image\n(Hamming 5 Pixels)"'
...
if __name__ == '__main__':
    ...
    for i in range( count ):
        command = './plot_pixel_image.py -s ' + sday + \
            ' -e ' +  eday + ' -l ' + ps[i] + ' -p ' + prefix + \
            ' -c ' + pc[i] +  \
